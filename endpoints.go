...
func (e *endpoints) generateOpenApiSchema(config OpenApiGeneratorConfig) (openapi3.T, error) {
	...
	// Convert JSON Schema definitions to OpenAPI schemas
	...
	for _, api := range e.api {
		...
		// Generate request schema if Request is provided
		var requestSchemaRef *openapi3.SchemaRef
		if api.Request != nil {
			reqSchema := jsonschema.Reflect(api.Request)
			// Add definitions to allDefs if not already present
			for k, v := range reqSchema.Definitions {
			...
			if reqSchema.Ref != "" {
				ref := strings.Replace(reqSchema.Ref, "#/$defs/", "#/components/schemas/", 1)
				requestSchemaRef = &openapi3.SchemaRef{Ref: ref}
			} else {
				// If no ref, convert the schema directly
				convertedSchema := convertJSONSchemaDefToOpenAPI(reqSchema, allDefs)
				requestSchemaRef = &openapi3.SchemaRef{Value: convertedSchema}
			}
		}

		// Generate response schema if Response is provided
		var responseSchemaRef *openapi3.SchemaRef
		if api.Response != nil {
			respSchema := jsonschema.Reflect(api.Response)
			// Add definitions to allDefs if not already present
			for k, v := range respSchema.Definitions {
				if _, exists := allDefs[k]; !exists {
					allDefs[k] = v
					// Also add to openAPISchemas
					openAPISchemas[k] = &openapi3.SchemaRef{
			...
			// Convert $ref from #/$defs/TypeName to #/components/schemas/TypeName
			if respSchema.Ref != "" {
				ref := strings.Replace(respSchema.Ref, "#/$defs/", "#/components/schemas/", 1)
				responseSchemaRef = &openapi3.SchemaRef{Ref: ref}
			} else {
				// If no ref, convert the schema directly
				convertedSchema := convertJSONSchemaDefToOpenAPI(respSchema, allDefs)
				responseSchemaRef = &openapi3.SchemaRef{Value: convertedSchema}
			}
		}
