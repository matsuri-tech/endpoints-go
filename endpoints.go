...
// RegisterSchemaOverride registers a custom JSON schema for the given type,
// overriding the schema that would be generated by reflection.
// Use this for types that implement json.Marshaler/json.Unmarshaler in a way
// that differs from their Go type (e.g., a uint that marshals to a string).
//
// Alternatively, types can implement the JSONSchemaer interface directly:
//
//	func (MyType) JSONSchema() *jsonschema.Schema { return &jsonschema.Schema{Type: "string"} }
func (e *endpoints) RegisterSchemaOverride(t reflect.Type, s *jsonschema.Schema) {
	if e.schemaOverrides == nil {
		e.schemaOverrides = make(map[reflect.Type]*jsonschema.Schema)
	}
	e.schemaOverrides[t] = s
}
...
func (e *endpoints) generateSchemaRef(typ any, allDefs jsonschema.Definitions, openAPISchemas openapi3.Schemas, typeToFinalName map[reflect.Type]string) *openapi3.SchemaRef {
	...
	overrides := e.schemaOverrides
	if overrides == nil {
		overrides = map[reflect.Type]*jsonschema.Schema{}
	}

	schema, typeReg := reflectWithTypeRegistry(typ, overrides)
...
func (e *endpoints) generateAPIList(version string, typeToFinalName map[reflect.Type]string) *orderedmap.OrderedMap {
	...
	for _, v := range e.api {
		...
		if len(v.Versions) == 0 || v.Versions.Includes(version) {
			apis.Set(v.Name, v.generatedApi(e.schemaOverrides, typeToFinalName))
		}
	}
	return apis
}
...
func (e *endpoints) collectAllDefs() []defsEntry {
	...
	for _, api := range e.api {
		if api.Request != nil {
			s, typeReg := reflectWithTypeRegistry(api.Request, overrides)
			entries = append(entries, defsEntry{defs: s.Definitions, typeReg: typeReg})
		}
		if api.Response != nil {
			s, typeReg := reflectWithTypeRegistry(api.Response, overrides)
			entries = append(entries, defsEntry{defs: s.Definitions, typeReg: typeReg})
		}
	}
	return entries
